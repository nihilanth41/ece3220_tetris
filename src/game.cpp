...
Game::Game(Grid *pGrid, Tetromino *pPieces, int pScreenHeight) {
...
void Game::InitGame()
{
    ...
    mPosX           = (GRID_WIDTH / 2) + mPieces->GetXInitialPosition (mPiece, mRotation);
    mPosY           = mPieces->GetYInitialPosition (mPiece, mRotation);
 
    //  Next piece
    mNextPiece      = GetRand (0, 6);
    mNextRotation   = GetRand (0, 3);
    mNextPosX       = GRID_WIDTH + 5;
    mNextPosY       = 5;   
}
...
void Game::DrawPiece (int pX, int pY, int pPiece, int pRotation)
{
    ...
    for (int i = 0; i < TETRO_BOUND; i++)
    {
        for (int j = 0; j < TETRO_BOUND; j++)
        {
...
void Game::DrawBoard ()
{
    ...
    int mX2 = BOARD_POSITION + (BLOCK_SIZE * (GRID_WIDTH / 2));
    int mY = mScreenHeight - (BLOCK_SIZE * GRID_HEIGHT);
